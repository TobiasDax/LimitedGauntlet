...
Either way, the default stack is reachable through the Cloudflare Tunnel hostname configured for `http://172.30.0.3:8080`; the app port is deliberately not published on the host. Set `CLOUDFLARE_TUNNEL_TOKEN` and `SESSION_COOKIE_SECURE=true` in `.env` before first boot. For a direct LAN or existing reverse-proxy deployment, use the explicit alternatives in [`docs/deployment.md`](docs/deployment.md). No manual database setup — migrations apply automatically before the server starts.

**Creating your first account:** signup is closed by default (`ALLOW_SIGNUP=false` in `.env.example`) — this app is meant for an existing group, not an open public signup form. To create your organization, set `ALLOW_SIGNUP=true` in `.env`, restart (`docker compose up -d`), open the configured tunnel hostname and sign up once, then set it back to `false` and restart again. (If you're migrating from spreadsheets/docs instead of starting fresh, see [History import](#history-import) below — it creates your first organization directly, no signup step needed.) Once that first organizer exists, add co-organizers from Settings → Organizers (needs SMTP configured) — no need to reopen signup for teammates.
