...
- **Outbound webhooks**: an HMAC-signed HTTP POST on round started/extended/completed and pairings-posted — drive a Home Assistant automation (or anything that accepts a webhook) off live tournament state. Off unless a URL is set — see [`docs/deployment.md`](docs/deployment.md#9-optional-outbound-webhooks-home-assistant-etc).
