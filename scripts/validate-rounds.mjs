...
function tally(matches) {
  ...
  for (const [a, b, ga, gb, gd = 0] of matches) {
    ...
    if (ga > gb) {
      add(points, a, POINTS_WIN);
      add(points, b, POINTS_LOSS);
    } else if (gb > ga) {
      add(points, b, POINTS_WIN);
      add(points, a, POINTS_LOSS);
    } else {
      add(points, a, POINTS_DRAW);
      add(points, b, POINTS_DRAW);
      drawCount++;
    }
  }
...
for (const pod of pods) {
  ...
  for (const p of players) {
    const gotPts = points.get(p) ?? 0,
      wantPts = pod.expected[p];
  ...
  if (probs.length) {
    ok = false;
    console.log(`\n[MISMATCH] ${pod.tournament} / ${pod.pod}`);
    probs.forEach((x) => console.log(x));
  } else
    console.log(
      `[OK] ${pod.pod} — ${players.length} players, ${drawCount} drawn match(es)${pod.gwp ? " (points+GW%)" : " (points only)"}`,
    );
}
console.log(
