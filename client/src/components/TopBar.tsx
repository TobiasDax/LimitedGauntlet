  ...
  return (
    <div className="border-border sticky top-0 z-30 border-b bg-bg/85 backdrop-blur">
      ...
      >
        ...
        {/* Mobile menu panel */}
        {open && (
          <nav id="mobile-nav" className="order-last flex w-full flex-col gap-1 pt-1 sm:hidden" aria-label="Primary">
            {orgSlot && <div className="border-border mb-1 border-b pb-2">{orgSlot}</div>}
            {navItems.map((item) => (
              <NavLink
                key={item.to}
                to={item.to}
