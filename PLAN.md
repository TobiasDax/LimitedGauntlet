...
**Tokens (PI-72)**: an *opt-in per-org* player currency (`Organization.tokensEnabled`, default off) tracked as an append-only `TokenTransaction` ledger — balance is `SUM(delta)`. Players earn tokens automatically for playing a pod (participation) and for finishing place (a configurable `{fromPlace,toPlace,tokens}` list on the tournament, overridable per pod); `syncPodTokenAwards` reconciles a pod's auto entries idempotently whenever its standings or completion state change. Organizers hand-adjust balances (add/deduct/"set to X") from a Tokens tab on the player page; the balance is public, the ledger is private (organizers + the player's own PI-52 account). The prize wall the tokens are spent on lives outside this app.

**Player accounts (PI-52)**: a *second, optional* auth surface. Auth columns (`email`/`passwordHash`/`authVersion`) live directly on `Player` — null on a plain roster entry. An organizer invites a roster player by email (`PlayerInvite`, mirrors the co-organizer invite); the player sets a password and logs in at `/o/<slug>/player` to check themselves into tournaments and report their own match results (`requirePlayerAuth`, session keys `playerId`/`playerAuthVersion`, kept independent of `organizerId`). This never adds a login wall to the open-by-default public pages — it only unlocks a player's own writes; a logged-in player *does* pass the optional PI-27 public-password lock for their own org. `Player` and `OrganizerAccount` stay distinct rows for the same human (no linking, no promotion path — an organizer who plays uses the organizer UI; a player who becomes an organizer gets a separate `OrganizerAccount` via PI-34). Result confirmation is deliberately lightweight for v1: either player may enter/overwrite the score while the round is `ACTIVE`, and the organizer is the final authority on completion.
